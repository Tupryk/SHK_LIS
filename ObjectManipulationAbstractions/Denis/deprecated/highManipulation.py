...
class RobotMan():
    ...
    def getNumberObjectives(self):
        return len(list((self.komo.report()).keys()))
    
    def getObjectiveTypes(self):
        return list((self.komo.report()).keys())

    # not needed probably
    # def getObjectiveInformation(self):
    #     return [self.komo.report()[list(self.komo.report().keys())[i]] for i in range(len(list(self.komo.report().keys())))]
