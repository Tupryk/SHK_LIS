...
def get_model_input(points, normals, sample_point, coor_frame, verbose=0):
    C = ry.Config()
    # C.addFile("justGripper.g")
    C.getJointState() # This needs to be done in order for C.eval to work

    r = R.from_matrix(coor_frame)
    C.addFrame("Marker").setShape(ry.ST.marker, [.02]).setPosition(sample_point).setQuaternion(r.as_quat())
