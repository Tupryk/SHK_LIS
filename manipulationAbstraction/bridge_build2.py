...
def roof_block(block: str, base_block: str, support_block: str) -> manip.ManipulationModelling:
    ...
    M.komo.addObjective([2], ry.FS.negDistance, [block, support_block], ry.OT.ineq, [-1e1], [-.002])
    M.komo.addObjective([2], ry.FS.negDistance, [block, base_block], ry.OT.ineq, [-1e1], [-.002])
...
def fill_pick_place_gaps_and_run(manipulation_model: manip.ManipulationModelling, box: str):
...
instructions = [base_block, support_block, roof_block]
problem = base_block
n_success = 0
...
while n_success < 3 and attempts_per_instruction < max_attempts_per_instruction:
    ...
    # Solve the full path
    if not fill_pick_place_gaps_and_run(M, box_order[n_success]):
